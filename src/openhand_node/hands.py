            ...
            for i in range(num_servos):
                ...
                if series == "RX":
                    if (self.motorDir[i] == 1 and enc > 512) or (self.motorDir[i] == -1 and enc < 512):
                        ...
                        _ = input("Your encoder position for motor index " + str(
                ...
                # These would then be the MX and XM motors
                    ...
                    print("Failsafe is incorporated to prevent gear shear in Model O")
                    print("Motor encoder postion: ", enc)
                    _ = input("As an XM Motor, we can automatically fix this issue for motor " + str(
                        i) + ", proceed? [ENTER]")
...
class Model_O(OpenHand):
    ...
    adduct_amount = 0.37
    motorMax = [motorMin[0] + adduct_amount, motorMin[1] + max_close, motorMin[2] + max_close,
                motorMin[3] + max_close]  # RX and MX motors may use an offset of 0.48 instead of 0.40

    HAND_HEIGHT = 0.14
    WRIST_OFFSET = -np.pi / 4

    def __init__(self, port="/dev/ttyUSB0", s1=2, s2=1, s3=3, s4=4, dyn_model="RX", s1_min=motorMin[0],
                 s2_min=motorMin[1], s3_min=motorMin[2], s4_min=motorMin[3], abduction_limit=1):
        ...
        if (mot_offset != self.motorMin):  # update motor mins if initialized to different values
            ...
            self.motorMax = [self.motorMin[0] + self.adduct_amount, self.motorMin[1] + self.max_close,
                             self.motorMin[2] + self.max_close, self.motorMin[3] + self.max_close]
    ...
    def pinch_object_move(self, delta_amnt=0.03, left=True,
                          down=False):  # These cannot be the same or else we will move diagonal
        adduct_loc, enc = self.readHand()
        if (adduct_loc[0] > 0.05):
    ...
    def torqueMotor(self, index, val, pos_val=None):
        ...
        if pos_val is None:
            ...
            enc = int(
                (pos_val * (self.motorMax[index].modes - self.motorMin[index]) + self.motorMin[index]) * s.settings[
                    'max_encoder'])
...
class Model_T42(OpenHand):
    ...
    def __init__(self, port="/dev/ttyUSB0", s1=1, s2=2, dyn_model="RX",
                 s1_min=motorMin[0], s2_min=motorMin[1], motorDir=None):
        # s1: "forefinger"
        # s2: "thumb"
        if motorDir is None:
            motorDir = [1, 1]
    ...
    def followTrajectory(self):  # position-based closing mechanism
        act1inputs = np.array(
            [0.6777, 0.6854, 0.6820, 0.6762, 0.6705, 0.6531, 0.6363, 0.6256, 0.6162, 0.6053, 0.6012, 0.5963, 0.5935,
             0.5905, 0.5888, 0.5864, 0.5846, 0.5815, 0.5782, 0.5729, 0.5654, 0.5555, 0.5467, 0.5381, 0.5317, 0.5283,
             0.5274, 0.5272, 0.5271, 0.5232, 0.5179, 0.5077, 0.4962, 0.4845, 0.4764, 0.4695, 0.4669, 0.4654, 0.4624,
             0.4589, 0.4556, 0.4527, 0.4502, 0.4493, 0.4491, 0.4490, 0.4490, 0.4490, 0.4490, 0.4490])
        act2inputs = np.array(
            [0.6629, 0.6726, 0.6733, 0.6751, 0.6788, 0.6784, 0.6780, 0.6791, 0.6796, 0.6753, 0.6741, 0.6728, 0.6722,
             0.6717, 0.6716, 0.6717, 0.6722, 0.6726, 0.6738, 0.6751, 0.6756, 0.6754, 0.6758, 0.6758, 0.6784, 0.6818,
             0.6855, 0.6858, 0.6858, 0.6827, 0.6792, 0.6762, 0.6761, 0.6759, 0.6728, 0.6675, 0.6617, 0.6560, 0.6522,
             0.6514, 0.6524, 0.6533, 0.6535, 0.6520, 0.6504, 0.6497, 0.6492, 0.6491, 0.6490, 0.6490])
